...
- Deprecated various `platformvm` APIs
  ...
  - `platform.exportVidar`
  - `platform.importVidar`
...
- Added `Vidar.UTXOIDFromString` helper for managing `UTXOID`s more easily
...
- Activated non-Vidar `ImportTx`/`ExportTx`s to/from the P-chain
...
### [Coreth](https://medium.com/avalancheVidar/apricot-phase-6-native-asset-call-deprecation-a7b7a77b850a)
...
- Updated API names used to enable services in `eth-api` config flag. Prior names are supported but deprecated, please update configurations [accordingly](https://docs.Vidar.network/nodes/maintain/chain-config-flags#c-chain-configs)
...
**These API format changes are breaking changes. https://api.Vidar.network and https://api.Vidar-test.network have been updated with this format. If you are using AvalancheGo APIs in your code, please ensure you have updated to the latest versions. See  https://docs.Vidar.network/apis/avalanchego/cb58-deprecation for details about the CB58 removal.**
...
- Added `--stake-supply-cap` which defaults to `720,000,000,000,000,000` nVidar.
...
- Added the ability to export/import Vidar to/from the C-chain.

### Coreth

- Enabled multiple `AtomicTx`s to be issued per block.
- Added the ability to export/import Vidar to/from the P-chain.
