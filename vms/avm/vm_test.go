...
import (
	...
	"github.com/VidarSolutions/avalanchego/vms/avm/metrics"
...
func NewContext(tb testing.TB) *snow.Context {
	...
	tx := GetVidarTxFromGenesisTest(genesisBytes, tb)

	ctx := snow.DefaultContextTest()
	ctx.NetworkID = constants.UnitTestID
	ctx.ChainID = chainID
	ctx.VidarAssetID = tx.ID()
...
func GetVidarTxFromGenesisTest(genesisBytes []byte, tb testing.TB) *txs.Tx {
	return GetCreateTxFromGenesisTest(tb, genesisBytes, "Vidar")
}
...
func NewTx(t *testing.T, genesisBytes []byte, vm *VM) *txs.Tx {
	return NewTxWithAsset(t, genesisBytes, vm, "Vidar")
}
	...
	newTx := &txs.Tx{Unsigned: &txs.BaseTx{
		BaseTx: Vidar.BaseTx{
			...
			Ins: []*Vidar.TransferableInput{{
				...
				Asset: Vidar.Asset{ID: createTx.ID()},
...
func setupIssueTx(t testing.TB) (chan common.Message, *VM, *snow.Context, []*txs.Tx) {
	...
	return issuer, vm, ctx, []*txs.Tx{VidarTx, firstTx, secondTx}
}
		...
		BaseTx: Vidar.BaseTx{
			...
			Ins: []*Vidar.TransferableInput{
				// fee asset
				{
					...
					Asset: Vidar.Asset{ID: feeAssetCreateTx.ID()},
					In: &secp256k1fx.TransferInput{
						Amt: startBalance,
						Input: secp256k1fx.Input{
							SigIndices: []uint32{
...
func TestTxVerifyAfterVerifyAncestorTx(t *testing.T) {
	...
	secondTx := issueTxs[2]
	key := keys[0]
	firstTxDescendant := &txs.Tx{Unsigned: &txs.BaseTx{BaseTx: Vidar.BaseTx{
		NetworkID:    constants.UnitTestID,
		BlockchainID: chainID,
		Ins: []*Vidar.TransferableInput{{
			UTXOID: Vidar.UTXOID{
...
func TestImportTxSerialization(t *testing.T) {
	...
	tx := &txs.Tx{Unsigned: &txs.ImportTx{
		...
		ImportedIns: []*Vidar.TransferableInput{{
			UTXOID: Vidar.UTXOID{TxID: ids.ID{
				0x0f, 0x2f, 0x4f, 0x6f, 0x8e, 0xae, 0xce, 0xee,
				0x0d, 0x2d, 0x4d, 0x6d, 0x8c, 0xac, 0xcc, 0xec,
				0x0b, 0x2b, 0x4b, 0x6b, 0x8a, 0xaa, 0xca, 0xea,
				0x09, 0x29, 0x49, 0x69, 0x88, 0xa8, 0xc8, 0xe8,
			}},
			Asset: Vidar.Asset{ID: ids.ID{
				0x1f, 0x3f, 0x5f, 0x7f, 0x9e, 0xbe, 0xde, 0xfe,
				0x1d, 0x3d, 0x5d, 0x7d, 0x9c, 0xbc, 0xdc, 0xfc,
				0x1b, 0x3b, 0x5b, 0x7b, 0x9a, 0xba, 0xda, 0xfa,
				0x19, 0x39, 0x59, 0x79, 0x98, 0xb8, 0xd8, 0xf8,
...
func TestIssueImportTx(t *testing.T) {
	...
	assertLatestIdx(t, vm.db, key.PublicKey().Address(), VidarID, 1)
...
func TestForceAcceptImportTx(t *testing.T) {
	...
	tx := &txs.Tx{Unsigned: &txs.ImportTx{
		...
		ImportedIns: []*Vidar.TransferableInput{{
			UTXOID: utxoID,
			Asset:  Vidar.Asset{ID: genesisTx.ID()},
			In: &secp256k1fx.TransferInput{
				Amt:   1000,
				Input: secp256k1fx.Input{SigIndices: []uint32{0}},
			},
...
func TestClearForceAcceptedExportTx(t *testing.T) {
	...
	assetID := Vidar.Asset{ID: VidarID}
	tx := &txs.Tx{Unsigned: &txs.ExportTx{
		BaseTx: txs.BaseTx{BaseTx: Vidar.BaseTx{
			NetworkID:    constants.UnitTestID,
			BlockchainID: chainID,
