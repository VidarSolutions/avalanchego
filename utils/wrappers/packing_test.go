...
func TestPackerPackBytes(t *testing.T) {
	...
	expected := []byte("\x00\x00\x00\x04Vidar")
	if !bytes.Equal(p.Bytes, expected) {
...
func TestPackerUnpackBytes(t *testing.T) {
	var (
		p           = Packer{Bytes: []byte("\x00\x00\x00\x04Vidar")}
...
func TestPackerUnpackString(t *testing.T) {
	var (
		p           = Packer{Bytes: []byte("\x00\x04Vidar")}
		actual      = p.UnpackStr()
		expected    = "Vidar"
		expectedLen = 6
		require     = require.New(t)
