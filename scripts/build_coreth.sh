...
if [[ ! -d "$coreth_path" ]]; then
  go get "github.com/VidarSolutions/coreth@$coreth_version"
fi
...
cd "$coreth_path"
go build $build_args -ldflags "-X github.com/VidarSolutions/coreth/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
